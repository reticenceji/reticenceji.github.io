...
他的编程实践也是简单的。通常只需要一个 **哈希表** + **双向链表**。哈希表保存 数据和数据对应的链表位置，链表则 暗示了数据的上次访问时间——每次读写数据都把数据移动到链表头，满了则删除链表尾对应的数据。

> [!NOTE]
> 为什么要使用双向链表？如果使用单向链表，将最近访问的数据从链表中移动到链表头的代价是O(n)，不能简单的完成删除操作了。
